...
# Read in Dataset
dataset_path = "G:\\SJTU\\2021-7\\nus_project\\Dataset_1"
processed_saved_path="G:\\SJTU\\2021-7\\nus_project\\processed_img"
masked_saved_path="G:\\SJTU\\2021-7\\nus_project\\masked_img"
X = []
y = []
X_masked=[]
X_processed=[]
...
#         if i<=30:
#             face_encoding=face_recognition.face_encodings(gaussian)
#             temp_model_masked.append(face_encoding)
#         # if i<10 and j<10:
...
#     model_masked.append(temp_model_masked)
...
for subject_name in os.listdir(masked_saved_path):
    ...
    for img_name in os.listdir(subject_images_dir):
        ...
        if(i<=30):
            face_encoding=face_recognition.face_encodings(img)
            temp_masked.append(face_encoding)
        # write code to read each 'img'

        # add the img to temp_x_list
    X_masked.append(temp_x_list)
    model_masked.append(temp_masked)


# test the model
test_img=io.imread("G:\\SJTU\\2021-7\\nus_project\\processed_img\\s01\\01.jpg")
